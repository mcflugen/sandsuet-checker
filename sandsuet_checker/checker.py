...
TIME_NAMES = {"time", "t"}
VERTICAL_NAMES = {"z", "depth", "elevation", "height", "level", "lev", "alt", "altitude"}
NORTH_SOUTH_NAMES = {"y", "lat", "latitude", "northing", "north", "south", "southing", "n", "s"}
EAST_WEST_NAMES = {"x", "lon", "longitude", "easting", "westing", "east", "west", "e", "w"}
...
class SandsuetChecker:
    ...
    def check_identity_mapping(self):
        ...
        for group in _all_groups(self.ds):
            for dim_name in group.dimensions:
                if dim_name not in group.variables and dim_name not in self.ds.variables:
                    loc = group.path if hasattr(group, "path") else "root"
                    failed.append(f"'{dim_name}' (in {loc})")
        if failed:
            self._log("Identity mapping", "FAIL",
                      f"Dimensions without a coordinate variable: {', '.join(failed)}.")
        else:
            self._log("Identity mapping", "PASS",
    ...
    def check_coordinates(self):
        for dim_name in self.ds.dimensions:
            ...
            if is_time:
                if is_increasing:
                    self._log("Temporal monotonicity", "PASS",
                              f"Temporal coordinate '{dim_name}' is monotonically increasing.")
                else:
                    self._log("Temporal monotonicity", "FAIL",
                ...
                if is_increasing or is_decreasing:
                    ...
                    self._log("Monotonicity", "PASS",
                              f"Coordinate '{dim_name}' is monotonically {direction}.")
                else:
                    self._log("Monotonicity", "FAIL",
    ...
    def check_variable_attributes(self):
        ...
        for var in _all_variables(self.ds):
            ...
            if not is_coord and not (hasattr(var, "description") or hasattr(var, "long_name")):
                missing_desc.append(var.name)

        if missing_units:
            self._log("Attribute presence", "FAIL",
                      f"Variables missing 'units': {', '.join(missing_units)}.")
        else:
            self._log("Attribute presence", "PASS", "All variables have 'units' attribute.")
        ...
        if missing_desc:
            ...
            self._log("Attribute presence", "PASS",
    ...
    def check_unit_consistency(self):
        if not CF_UNITS_AVAILABLE:
            self._log("Unit consistency", "WARN",
        ...
        if invalid:
            ...
            self._log("Unit consistency", "PASS",
                      "All variable units are UDUNITS-compliant.")
